...
# Install Kong Gateway 3.11.x ubunut
...
sudo apt-get update && sudo apt-get install -y kong-enterprise-edition=3.11.0.3
...
services:
  ...
  kong-bootstrap:
    image: '${GW_IMAGE:-kong/kong-gateway:3.11.0.3}'  # Kong Gateway image (default to latest version)
  ...
  kong-cp:
    image: '${GW_IMAGE:-kong/kong-gateway:3.11.0.3}'  # Main Kong Gateway Control Plane (default to latest version)
